...
SELECT 
	...
	count(*) AS rented_booth_count
FROM vendor 
INNER JOIN vendor_booth_assignments 
	ON vendor.vendor_id = vendor_booth_assignments.vendor_id
GROUP BY vendor_booth_assignments.vendor_id
ORDER BY vendor_booth_assignments.vendor_id;
...
VALUES((SELECT IFNULL(MAX(vendor_id), 0) + 1 FROM temp.new_vendor), 'Thomass Superfood Store', 'Fresh Focused store', 'Thomas', 'Rosenthal');
...
SELECT 
	...
	SUM(quantity * cost_to_customer_per_qty) AS total_spent
FROM customer_purchases
GROUP BY customer_id, market_month, market_year
HAVING market_month = '04' AND market_year = '2022';
